    ...
    for ix,times in enumerate(timesArr):
        ...
        if candle_datas["status"]=="0000":
            ...
            for ix in range(len(x_data_start)):
                x_dataset.append(sum([x_data_start[ix], x_data_end[ix],x_data_high[ix], x_data_low[ix]]) / 4)
                ##???? (1,60)
            ...
            if scaler:
                ...
                print("스케일러가 존재하지 않습니다. 해당 가상화폐를 훈련 후 다시 실행하세요")
        else:
            print("데이터 수신 불량")
    # 루프종료부
    print("xxxxxxxxxxxxxxxxxxxxxxxxxxxx",coinname,"xxxxxxxxxxxxxxxxxxxxxxxxxxxx")
    print("현재최고가:",round(cur_high[0]/len(timesArr),4))
    print("현재최저가:", round(cur_low[0]/len(timesArr),4))
    print("현재시작가:",round(cur_start[0]/len(timesArr),4))
    print("현재종료가:",round(cur_end[0]/len(timesArr),4))
    # cur_summary = round(((cur_high[0]+cur_low[0]+cur_start[0]+cur_end[0])/len(timesArr))/4,4)
    cur_summary = round(((cur_high[0] / len(timesArr)) + (cur_low[0] / len(timesArr)) + (cur_start[0] / len(timesArr)) + (cur_end[0] / len(timesArr))) / 4, 4)

    print(" :::::: 현재요약가 :::::: ",cur_summary)
    res_summary=round(res_summary/len(timeArr),4)
    print(" :::::: 최종 요약 결과 :::::: ",res_summary,"원 예상")
...
def predict_coinprice(coinname,ctime):
    pass
    tf.keras.models.load_model(r"models\{}_{}_rnnmodel.keras".format(coinname,ctime))
    # x_data =
names=getInitName()
